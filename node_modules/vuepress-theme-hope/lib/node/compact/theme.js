...
const handleBlogOptions = (blogOptions) => {
    if ("links" in blogOptions) {
        logger.warn('"blog.links" options is deprecated, please use "blog.medias" instead');
        blogOptions["medias"] = blogOptions["links"];
        delete blogOptions["links"];
    }
    if ("perPage" in blogOptions) {
        logger.warn('"blog.perPage" options is deprecated, please use "blog.articlePerPage" instead');
        blogOptions["articlePerPage"] = blogOptions["perPage"];
        delete blogOptions["perPage"];
    }
    if ("autoExcerpt" in blogOptions) {
        logger.error('"blog.autoExcerpt" options is no longer supported, please use "plugins.blog.autoExcerpt" instead');
...
const handleFooterOptions = (options) => {
    if (typeof options["footer"] === "object" && options["footer"]) {
        ...
        if ("copyright" in footer) {
            logger.warn('"footer.copyright" options is deprecated, please use "copyright" instead');
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            options["copyright"] = footer["copyright"];
        }
        if ("display" in footer) {
            logger.warn('"footer.display" options is deprecated, please use "displayFooter" instead');
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            options["displayFooter"] = footer["display"];
        }
        if ("content" in footer) {
            logger.warn('"footer.content" options is deprecated, please use "footer" instead');
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
 ...
 * @deprecated You should use V2 standard options and avoid using it
 */
export const convertThemeConfig = (themeOptions) => {
    // ensure plugins
    const plugins = (themeOptions["plugins"] =
        themeOptions["plugins"] || {});
    DEPRECATED_THEME_OPTIONS.forEach(([deprecatedOption, newOption]) => deprecatedLogger({
    ...
    if (typeof themeOptions["encrypt"] === "object" && themeOptions["encrypt"]) {
        ...
        if ("global" in encrypt && typeof encrypt["global"] !== "boolean") {
            logger.warn('Setting admin password with "encrypt.global" in V1 is deprecated in V2, please use "encrypt.admin" instead.');
            encrypt["admin"] = encrypt["global"];
        }
        if ("status" in encrypt) {
            logger.warn('"encrypt.status" is deprecated, please use "encrypt.global" instead.');
    ...
    if ("locales" in themeOptions &&
        ...
        Object.values(themeOptions["locales"]).forEach((localeConfig) => {
            ...
            DROPPED_THEME_OPTIONS.forEach((item) => droppedLogger(localeConfig, ...item));
            // handle navbar
            if ("navbar" in localeConfig)
                localeConfig["navbar"] = convertNavbarConfig(localeConfig["navbar"]);
            // handle sidebar
            if ("sidebar" in localeConfig)
                localeConfig["sidebar"] = convertSidebarConfig(localeConfig["sidebar"]);
            // handle footer
