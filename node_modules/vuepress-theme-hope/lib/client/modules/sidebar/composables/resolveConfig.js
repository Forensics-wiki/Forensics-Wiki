...
import { useAutoLink, useThemeLocaleData, } from "@theme-hope/composables/index.js";
import { resolvePrefix } from "./utils.js";
/**
 * Util to transform page header to sidebar item
 */
export const headerToSidebarItem = (header, headerDepth) => {
...
export const headersToSidebarItemChildren = (headers, headerDepth) => headerDepth > 0
    ? headers.map((header) => headerToSidebarItem(header, headerDepth - 1))
    : [];
/**
 * Resolve sidebar items if the config is `heading`
 */
export const resolveHeadingSidebarItems = (headerDepth) => {
    const page = usePageData();
 ...
 */
export const resolveArraySidebarItems = (sidebarConfig, headerDepth, prefix = "") => {
    const page = usePageData();
        ...
        return {
            ...
            childItem.link === route.path
                ...
                page.value.headers[0]?.level === 1
                    ...
                    : page.value.headers, headerDepth)
                : [],
        };
    };
 ...
 */
export const resolveMultiSidebarItems = (sidebarConfig, headerDepth) => {
    const route = useRoute();
    ...
    for (const base of keys) {
        if (decodeURI(route.path).startsWith(base)) {
            ...
            return matchedConfig
                ? resolveArraySidebarItems(matchedConfig === "structure" ? sidebarData[base] : matchedConfig, headerDepth, base)
                : [];
        }
    }
    ...
    return sidebarConfig === false
        ...
        : sidebarConfig === "heading"
            ...
            : sidebarConfig === "structure"
                ? resolveArraySidebarItems(sidebarData[routeLocale.value], headerDepth, routeLocale.value)
