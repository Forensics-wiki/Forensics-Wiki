import{chalk as $,fs as x}from"@vuepress/utils";import{Logger as P}from"vuepress-shared/node";import{isLinkHttp as T,removeEndingSlash as b,removeLeadingSlash as O}from"@vuepress/shared";import{SitemapStream as D}from"sitemap";const d=new P("vuepress-plugin-sitemap2"),S=({options:e,deprecatedOption:n,newOption:a,msg:s="",scope:p=""})=>{if(n in e){if(d.warn(`${$.magenta(n)} is ${$.yellow("deprecated")}${p?` in ${p}`:""}, please use "${$.magenta(a)}" instead.${s?`
...
Sitemap: ${p}${o}${l}
`;await x.writeFile(u,h,{flag:"w"}),d.succeed()}},k=(e,n=!1)=>a=>{n&&G(e),a.env.isDebug&&d.info(`Options: ${e.toString()}`);const s={name:"vuepress-plugin-sitemap2"};return e.hostname?{...s,onGenerated:p=>U(p,e)}:(d.error(`Option ${$.magenta("hostname")} is required!`),s)};export{k as sitemapPlugin};
