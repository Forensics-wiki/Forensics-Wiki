...
class CommunitySkin extends Component {
  ...
  De() {
    const { viewType, streamType } = this.n.$store;
    return viewType() === "audio" ? streamType().includes("live") ? "audio:live" : "audio" : streamType().endsWith("live") ? "video:live" : "video";
  }
  onAttach() {
    this.setAttributes({
      "data-audio": this.Ee.bind(this),
      "data-video": this.Fe.bind(this),
      "data-mobile": this.Ce.bind(this)
    });
  }
  Ee() {
  ...
  Ce() {
    const { breakpointX } = this.n.$store;
    return breakpointX() === "sm";
  }
  render() {
    const $layoutType = computed(this.De.bind(this)), $isMobile = computed(this.Ce.bind(this));
    return () => {
